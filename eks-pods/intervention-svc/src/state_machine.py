...
def _to_approved(cur, order_id: str, ctx: AuthContext, approval_side: str | None) -> dict:
    ...
    if side not in ("SOURCE", "TARGET", "FINAL"):
    ...
    cur.execute(
        ...
        """,
        (order_id, ctx.user_id, ctx.role, side),
        prepare=False,
    )
...
def _infer_side(cur, order_id: str, ctx: AuthContext) -> str:
    """ctx.role/scope 와 order 의 source/target 비교하여 SOURCE / TARGET / FINAL 추정.
    PUBLISHER_ORDER = hq FINAL 단독. hq-admin 은 명시 안 하면 FINAL.
    """
    cur.execute(
        """
    ...
    order_type, src_loc, tgt_loc, src_wh, tgt_wh = row
