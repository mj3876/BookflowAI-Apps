...
Glue 스크립트는 `BookFlowAI-Apps` 레포 변경 시 GitHub Actions가 자동으로  
`s3://bookflow-glue-scripts-354493396671/` 에 sync한다.  
Glue Job은 `ScriptLocation`으로 해당 S3 경로를 직접 참조하므로 재배포 불필요.
